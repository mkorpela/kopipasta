...
def mock_deps():
    ...
    with patch("kopipasta.mcp_server._load_config", return_value=MOCK_CONFIG) as mock_config, \
...
def test_apply_edits_permission_denied(mock_deps):
    ...
    with patch.object(Path, "exists", return_value=True), \
         patch.object(Path, "resolve", side_effect=lambda: MOCK_PROJECT_ROOT / "secret.py"):
